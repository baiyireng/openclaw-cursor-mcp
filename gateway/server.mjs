...
const server = createServer(async (req, res) => {
  try {
    ...
    if (req.method === "POST" && req.url === "/config") {
      ...
      return sendJson(res, 200, { ok: true, message: "Config saved. Please restart gateway to apply." });
    }

    if (req.method === "POST" && req.url === "/config/test-cloud") {
      const payload = await readJsonBody(req);
      const testResult = await testCloudConnection(payload, config);
    ...
    if (req.method === "POST" && req.url === "/chat") {
      ...
      const reply = await resolveReply(sessionId, message);
...
async function resolveReply(sessionId, message) {
  if (mode === "cli") {
    return runCliBackend(sessionId, message);
  }
  if (mode === "plugin") {
    return runPluginBackend(sessionId, message);
  }
  return `Gateway(mock): session=${sessionId}, message=${message}`;
}

async function runCliBackend(sessionId, message) {
  const cmd = resolveSetting("CURSOR_GATEWAY_CLI_CMD", "cli.command", "");
  if (!cmd) {
    throw new Error("CURSOR_GATEWAY_CLI_CMD is required in cli mode");
  }
  ...
  const { stdout } = await execFileAsync(cmd, args, {
    windowsHide: true,
    timeout: toInt(resolveSetting("CURSOR_GATEWAY_CLI_TIMEOUT_MS", "cli.timeoutMs", "60000"), 60000)
  });

  const text = stdout.trim();
  if (!text) {
    throw new Error("CLI backend returned empty stdout");
  }
  return text;
}

async function runPluginBackend(sessionId, message) {
  ...
  if (typeof mod.generateReply !== "function") {
    throw new Error(`Plugin ${specifier} must export async function generateReply(sessionId, message)`);
  }
  ...
  if (typeof reply !== "string" || !reply.trim()) {
    throw new Error(`Plugin ${specifier} returned invalid reply`);
  }
  return reply.trim();
}
...
async function testCloudConnection(payload, currentConfig) {
  try {
    ...
    if (!apiKey) {
      return { ok: false, message: "缺少 CURSOR_CLOUD_API_KEY，无法测试" };
    }
    ...
    return { ok: true, message: "Cloud 连接成功（已验证 create agent 权限）" };
  } catch (error) {
    return { ok: false, message: `Cloud 测试异常: ${String(error)}` };
  }
}
